/**
 * Manifest 纯数据校验入口（SDK 版）。
 *
 * 与宿主 src/plugins/manifest-validation.ts 共用同一份自动生成的
 * manifest.schema.json：结构、类型、枚举（deps 能力名单）和必填字段
 * 都在 Schema 里定义。Schema 校验不了的内容——入口文件存在性、符号
 * 链接越界、icon 规则、apiVersion 兼容性、id/SemVer 格式——由宿主
 * 加载器的文件系统与格式校验完成，两层必须同时保留。
 */
import Ajv from "ajv";
import type { PluginManifestInput } from "./api";
